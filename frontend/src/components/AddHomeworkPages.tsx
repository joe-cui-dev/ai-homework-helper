...
export function AddHomeworkPages({ disabled, remainingPages, remainingQuestions, error, completedPageCount, onSubmit }: AddHomeworkPagesProps) {
  ...
  const change = (next: ImageAttachmentValue[]) => {
    if (next.map((a) => a.id).join(",") !== attachments.map((a) => a.id).join(",")) setSubmissionId(crypto.randomUUID());
    setAttachments(next);
  };
  ...
  return <section className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 sm:p-6 space-y-3">
    ...
    <button disabled={disabled || attachments.length === 0} onClick={() => onSubmit(attachments.map((a) => a.dataUrl), submissionId)} className="px-5 py-2 rounded-xl bg-brand-600 text-white font-semibold disabled:opacity-50">{error ? "Retry adding pages" : "Add pages"}</button>
  </section>;
}
