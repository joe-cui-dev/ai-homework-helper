...
### Writing Plan
The turn-1 output of a Writing Session — the **parent's coaching plan** for this assignment, not the kid's outline. Tells the parent what success looks like, what to ask the child before writing, what pitfalls to anticipate, and what to do during writing. Holds a `modelAnswer` (in student voice at the inferred year level) gated behind a UI disclosure so the parent must opt in to see it. Genre and year level are inferred at turn 1 and locked for the session.
